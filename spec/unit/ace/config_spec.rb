...
RSpec.describe ACE::Config do
  ...
  let(:emptyconfig) { File.join(configdir, 'empty-ace-server.conf') }
  ...
  context 'with full config' do
    ...
    it 'reads port' do
      expect(config['port']).to eq(44633)
    end
  ...
  it "accepts only required config" do
    ...
    expect(config['port']).to be(44633)
  ...
  it "errors when ssl-cipher-suites is not an array" do
    expect {
      ACE::Config.new(base_config.merge('ssl-cipher-suites' => 'notanarray')).validate
  ...
  it "errors when file-server-conn-timeout is not an integer" do
    expect {
      ACE::Config.new(base_config.merge('file-server-conn-timeout' => '10')).validate
    }.to raise_error(Bolt::ValidationError, "Configured 'file-server-conn-timeout' must be a positive integer")
  end

  it "errors when file-server-conn-timeout is zero" do
    expect {
      ACE::Config.new(base_config.merge('file-server-conn-timeout' => 0)).validate
    }.to raise_error(Bolt::ValidationError, "Configured 'file-server-conn-timeout' must be a positive integer")
  end

  it "errors when file-server-conn-timeout is negative" do
    expect {
      ACE::Config.new(base_config.merge('file-server-conn-timeout' => -1)).validate
    }.to raise_error(Bolt::ValidationError, "Configured 'file-server-conn-timeout' must be a positive integer")
  end
end
